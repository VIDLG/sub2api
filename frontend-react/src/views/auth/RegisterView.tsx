        ...
        if (emailVerifyEnabled) {
          sessionStorage.setItem(
            ...
            JSON.stringify({
              ...
              promo_code: value.promoCode || undefined,
              invitation_code: value.invitationCode || undefined,
            }),
          )
    ...
    <AuthLayout footer={footer}>
      <div className="space-y-5">
        <div className="text-center">
          ...
          <p className="mt-1.5 text-sm text-gray-500 dark:text-dark-400">
        ...
        {linuxdoOAuthEnabled && (
          <div className="space-y-4">
            <a
              href={`/api/v1/auth/oauth/linuxdo/start?redirect=${encodeURIComponent('/dashboard')}`}
              className={`btn btn-secondary flex w-full items-center justify-center ${isLoading ? 'pointer-events-none opacity-50' : ''}`}
            >
          ...
          >
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <button
                      ...
                      className="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-dark-300"
                    >
                      {showPassword ? (
            ...
            {invitationCodeEnabled && (
              <div className="space-y-2">
                ...
                <div className="relative">
                  <KeyIcon
                    className={`absolute left-3.5 top-1/2 h-5 w-5 -translate-y-1/2 ${invitationValidation.valid ? 'text-green-500' : 'text-gray-400'}`}
                  />
                  <Input
                    id="invitation_code"
                    value={form.state.values.invitationCode}
                    onChange={(e) => handleInvitationCodeInput(e.target.value)}
                    disabled={isLoading}
                    className={`pl-11 pr-10 ${invitationValidation.valid ? 'border-green-500' : invitationValidation.invalid ? 'border-red-500' : ''}`}
                  ...
                  <div className="absolute right-3.5 top-1/2 -translate-y-1/2">
                    ...
                    {!invitationValidating && invitationValidation.invalid && (
                      <svg
                ...
                {invitationValidation.invalid && (
                  <p className="text-sm text-destructive">{invitationValidation.message}</p>
                )}
              </div>
            )}
            ...
            {promoCodeEnabled && (
              <div className="space-y-2">
                ...
                <div className="relative">
                  <GiftIcon
                    className={`absolute left-3.5 top-1/2 h-5 w-5 -translate-y-1/2 ${promoValidation.valid ? 'text-green-500' : 'text-gray-400'}`}
                  />
                  <Input
                    id="promo_code"
                    value={form.state.values.promoCode}
                    onChange={(e) => handlePromoCodeInput(e.target.value)}
                    disabled={isLoading}
                    className={`pl-11 pr-10 ${promoValidation.valid ? 'border-green-500' : promoValidation.invalid ? 'border-red-500' : ''}`}
